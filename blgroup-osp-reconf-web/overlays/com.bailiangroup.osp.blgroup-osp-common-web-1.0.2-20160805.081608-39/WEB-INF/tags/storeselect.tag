...
<%@ attribute name="userTag" type="java.lang.String" required="false" description="系统管理专用"%>
...
$("#${id}Button").click(function(){
	...
	var url="iframe:${ctx}/sys/omni/selectStore?typeStr="+typeStr+"&ids="+$("#${id}Id").val();
	top.$.jBox.open(url, "选择业态", 300, 420, {
		buttons:{"确定":"ok","关闭":true}, submit:function(v, h, f){
			if (v=="ok"){
				...
				var tagStr='${userTag}';
				if(tagStr!=null&&tagStr=='1'){
					var elementsTag=$('#storeTags_tagsinput [class="tag"]');
					if(elementsTag!=null){
						$(elementsTag).each(function(){
							$(this).remove();
						});
						$('#storeIdList').val('');
					}
