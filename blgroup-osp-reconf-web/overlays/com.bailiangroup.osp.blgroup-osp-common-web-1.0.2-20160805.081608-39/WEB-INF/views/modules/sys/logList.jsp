...
<body>
	...
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post" class="breadcrumb form-search">
		...
		<div>
			<label>用户登录名：</label><input id="createById" name="createById" type="text" maxlength="50" class="input-small" value="${createById}"/>
			<label>操作用户：</label><input id="createByName" name="createByName" type="text" maxlength="50" class="input-small" value="${createByName}"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="log">
			...
			<tr>
				...
				<c:if test="${not empty log.exception}"><br/>异常信息: ${log.exception }</c:if></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
