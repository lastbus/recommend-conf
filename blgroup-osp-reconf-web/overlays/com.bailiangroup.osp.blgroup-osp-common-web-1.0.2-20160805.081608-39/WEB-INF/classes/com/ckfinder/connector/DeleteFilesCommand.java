	...
	protected int getDataForXml() {
		...
		for (FilePostParam fileItem : this.files) {
			...
			try {
				...
				{
					//删除本地文件
					String filePath=configuration.getTypes().get(fileItem.getType()).getPath()+"/upload"+config.getFtpDir()+File.separator+fileItem.getType() + this.currentFolder+fileItem.getName();
					File delFile=new File(filePath);
					if(delFile.exists())
						delFile.delete();
					String ftpPath=File.separator + fileItem.getType() + this.currentFolder+fileItem.getName();
