...
import com.bailiangroup.osp.base.core.util.FileUtils;
	...
	private void prepareGetResponse(final HttpServletRequest request,
		...
		if ("Init".equals(command)){
			...
			//if (user!=null){
				...
				if (startupPath!=null){
					...
					if (ss.length==2){
						//图片文件夹管理，因采用只读mount，故这里需在图片服务器上创建并同步到本地
						//String path = "/userfiles/"+user.getId()+"/"+ss[0]+ss[1];
						//String path = "/userfiles"+baseDir+"/images"+ss[1];
						String path="/images"+ss[1];
						try {
							FtpClientUtil.createNestedDirectory(config.getFtpUrl(), config.getFtpPort(), config.getFtpUsername(), config.getFtpPassword(), path);
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						//创建临时上传目录
						path = "/upload"+baseDir+"/images"+ss[1];
		...
		// 快捷上传，自动创建当前文件夹，并上传到该路径
			...
			//if (user!=null){
				...
				/*String path = "/userfiles/"+baseDir+"/images"+(currentFolder!=null?currentFolder:"");
				String realPath = request.getSession().getServletContext().getRealPath(path);
				FileUtils.createDirectory(realPath);*/
				String path="/images"+(currentFolder!=null?currentFolder:"");
				try {
					FtpClientUtil.createNestedDirectory(config.getFtpUrl(), config.getFtpPort(), config.getFtpUsername(), config.getFtpPassword(), path);
				} catch (IOException e) {
					// TODO Auto-generated catch block
				...
				//创建临时上传目录
				path = "/upload"+baseDir+"/images"+(currentFolder!=null?currentFolder:"");
				String realPath = request.getSession().getServletContext().getRealPath(path);
				FileUtils.createDirectory(realPath);
			//}
		}
